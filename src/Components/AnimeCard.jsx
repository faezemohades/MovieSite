...
function AnimeCard({ movie }) {
    ...
    <Link to={`/Home/Video/${movie.name}/${movie.title.replaceAll(/\s+/g, "-").replaceAll('---', '-')}`} className="d-flex justify-content-center card-style gap-10" >
                <Card className="anime-card"  >
                    <Card.Img variant="top" src={movie.image} className="img-anime" alt="" />
                    <Card.Body>
                        <Card.Title  style={{ fontSize: "15px", lineHeight: "21px", wordBreak: "break-word" }}>{movie.title.slice(0,45)+".."}</Card.Title>
                    </Card.Body>
                </Card>
            </Link>
        </motion.div>
